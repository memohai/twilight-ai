...
//
// batchID is generated by the SDK, is unique per invocation, and reappears
// as ToolApprovalPause.BatchID when any call defers: tag persisted approval
// records with it, and reconciliation (finding approval rows whose pause was
// never persisted) has a stable correlation key on both sides. It is not an
// idempotency key — retrying a step mints a new one.
//
// calls lists the step's approval-gated, executable calls in tool-call
// order. The response is keyed by ToolCallID, and the SDK verifies it is
// complete: exactly one result per call, no unknown or duplicate IDs, and
// every Decision explicitly approved, rejected, or deferred — the zero
// value is an error here, never an implicit approval (a missed assignment
// in host code must not approve a gated tool). A handler error fails the
// whole batch before anything is emitted or executed, so a transactional
// handler that rolled back leaves no trace on either side.
//
// Mutually exclusive with WithApprovalHandler — configuring both is an
// error. Hosts without transactional persistence needs can keep the simpler
// per-call handler.
